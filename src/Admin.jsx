...
export default function Admin() {
  ...
  function login() {
    if (!adminPassword) { setError('Cargando configuración...'); return }
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", background: C.bg, minHeight: '100vh' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {cargando && <div style={{ textAlign: 'center', padding: '40px', color: C.textSub }}>Cargando...</div>}

        {restaurantes.map(r => {
          const s = stats[r.id] || { ordenes: 0, ventas: 0, activas: 0 }
          return (
            <div key={r.id} style={{ background: C.card, borderRadius: '16px', padding: '16px', marginBottom: '12px', border: '1px solid ' + (r.activo ? '#2D6A4F40' : C.border) }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div>
                  ...
                  <div style={{ fontSize: '10px', color: C.gold, marginTop: '4px', fontFamily: 'monospace' }}>/r/{r.slug}</div>
                </div>
                <button
                  onClick={() => toggleRestaurante(r.id, r.activo)}
                  style={{ padding: '5px 12px', borderRadius: '20px', border: '1px solid ' + (r.activo ? '#2D6A4F40' : '#C0392B40'), cursor: 'pointer', fontSize: '11px', fontWeight: '700', background: r.activo ? '#0D2318' : '#1A0808', color: r.activo ? C.green : C.red }}
                >
              ...
              <button onClick={() => abrirModalUsuarios(r)} style={{ width: '100%', background: C.bg2, color: C.silver, border: '1px solid ' + C.border, borderRadius: '10px', padding: '10px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', marginBottom: '10px' }}>
                👥 Gestionar usuarios
              </button>
      ...
      {modalRestaurante && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: C.bg2, borderRadius: '20px', padding: '24px', width: '100%', maxWidth: '420px', border: '1px solid ' + C.border }}>
            ...
            <input placeholder="Ej: Cocina mexicana · Providencia, GDL" value={nuevoRestaurante.descripcion}
      ...
      {modalUsuario && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
          <div style={{ background: C.bg2, borderRadius: '20px', padding: '24px', width: '100%', maxWidth: '420px', maxHeight: '90vh', overflowY: 'auto', border: '1px solid ' + C.border }}>
            ...
            {loadingUsuarios && <div style={{ textAlign: 'center', color: C.textSub, padding: '20px' }}>Cargando...</div>}
            {usuariosRestaurante.map(u => {
              ...
              return (
                <div key={u.id} style={{ background: C.card, borderRadius: '12px', padding: '12px 14px', marginBottom: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid ' + C.border }}>
                  ...
                  <button onClick={() => eliminarUsuario(u.id)} style={{ background: '#1A0808', color: C.red, border: '1px solid #C0392B40', borderRadius: '8px', padding: '6px 10px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>
                    Eliminar
                  </button>
                </div>
              )
